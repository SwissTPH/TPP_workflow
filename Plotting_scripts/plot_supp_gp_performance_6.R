...
############ Create figure and supplementary data file for GP performance - late follow-up ################
gp_perf = vector('list', 11)
gp_perf[[1]] = create_perf_report("~/MMC/TPP/simulations/MAB_once_3years_avg_prev/", 6, 
                                  "~/MMC/TPP/simulations/test_sets/test_MAB_once_3years_avg_prev/",
                                  "Anti-infective monoclonal \n(once per year)")
gp_perf[[2]] = create_perf_report("~/MMC/TPP/simulations/MAB_twice_sync_3years/", 6, 
                                  "~/MMC/TPP/simulations/test_sets/test_MAB_twice_sync_3years/",
                                  "Anti-infective monoclonal \n(twice per year)")
# TO DO: MAB once
gp_perf[[3]] = create_perf_report("~/MMC/TPP/simulations/cohort_MDA_short_MAB_once/", 6, 
                                  "~/MMC/TPP/simulations/test_sets/test_cohort_MDA_short_MAB_once/",
                                  "Anti-infective monoclonal + \nblood stage drug (once per year)")
gp_perf[[4]] = create_perf_report("~/MMC/TPP/simulations/cohort_MDA_short_MAB_twice/", 6, 
                                  "~/MMC/TPP/simulations/test_sets/test_cohort_MDA_short_MAB_twice/",
                                  "Anti-infective monoclonal + \nblood stage drug (twice per year)")
gp_perf[[5]] = create_perf_report("~/MMC/TPP/simulations/PEV_once_3years/", 6, 
                                  "~/MMC/TPP/simulations/test_sets/test_PEV_once_3years/",
                                  "Anti-infective vaccine \n(once per year)")
gp_perf[[6]] = create_perf_report("~/MMC/TPP/simulations/cohort_MDA_short_AIV/", 6, 
                                  "~/MMC/TPP/simulations/test_sets/test_cohort_MDA_short_AIV/",
                                  "Anti-infective vaccine + \nblood stage drug (once per year)")
gp_perf[[7]] = create_perf_report("~/MMC/TPP/simulations/TBV_once_3years/", 6, 
                                  "~/MMC/TPP/simulations/test_sets/test_TBV_once_3years/",
                                  "Transmission-blocking vaccine \n(once per year)")
gp_perf[[8]] = create_perf_report("~/MMC/TPP/simulations/cohort_MDA_short_TBV/", 6, 
                                  "~/MMC/TPP/simulations/test_sets/test_cohort_MDA_short_TBV/",
                                  "Transmission-blocking vaccine + \nblood stage drug (once per year)")
gp_perf[[9]] = create_perf_report("~/MMC/TPP/simulations/ATSB_once_3years/", 6, 
                                  "~/MMC/TPP/simulations/test_sets/test_ATSB_once_3years/",
                                  "Attractive targeted sugar baits \n(once per year)")
gp_perf[[10]] = create_perf_report("~/MMC/TPP/simulations/ATSB_twice_sync_3years/", 6, 
                                   "~/MMC/TPP/simulations/test_sets/test_ATSB_twice_sync_3years/",
                                   "Attractive targeted sugar baits \n(twice per year)")
gp_perf[[11]] = create_perf_report("~/MMC/TPP/simulations/preprandial_once_3years/", 6, 
...
error_plot = ggplot(df_perf, aes( x = Out_of_sample_ME)) +
    ...
    labs( x = "Mean absolute error (%)", y = "Count") + xlim(0,4)
...
plot_perf[[12]] = error_plot + annotation_custom(ggplotGrob(corr_plot), xmin = 2.5, xmax = 4, ymin = 5, ymax = 65) +
...
ggsave("~/MMC/TPP/figures/gp_performance/Supp_perf_6_updated.pdf",
...
write.table(df_perf, "~/MMC/TPP/figures/gp_performance/gp_perf_6_updated.txt", quote = FALSE, sep= "\t", col.names = TRUE, row.names = FALSE)
