##################################
# EIR to prevalence plots for patent infections
#
# created 28.07.2020
# monica.golumbeanu@unibas.ch
#################################

sim_folder = "~/MMC/TPP/simulations/test_sets/test_RDT/"
...
for(p_file in processing_files) {
    ...
    plot_df = rbind.data.frame(plot_df, OM_result[, c("Seasonality", "Biting_pattern", "EIR", "initial_prev_patent", "initial_prev_2_10_patent")])
}
...
# if(length(which(plot_df$initial_prev_patent == 0))>0) {
#     plot_df = plot_df[-which(plot_df$initial_prev_patent == 0),]
# }
...
stats_table = plot_df %>% group_by(Seasonality, Biting_pattern, EIR) %>% 
    dplyr::summarise(Median_0_99 = median(initial_prev_patent), 
                     Conf_95_low_0_99 = boxplot.stats(initial_prev_patent)$conf[1], 
                     Conf_95_high_0_99 = boxplot.stats(initial_prev_patent)$conf[2],
                     Median_2_10 = median(initial_prev_2_10_patent), 
                     Conf_95_low_2_10 = boxplot.stats(initial_prev_2_10_patent)$conf[1],
                     Conf_95_high_2_10 = boxplot.stats(initial_prev_2_10_patent)$conf[2])

ex_table = plot_df[,c("Seasonality", "Biting_pattern", "EIR", "initial_prev_patent", "initial_prev_2_10_patent")]
...
ggplot(ex_table_m, aes(x=as.factor(EIR), y=value*100, color = variable)) + 
    ...
    scale_color_manual(values=c("#56B4E9", "#E69F00"), 
                       name="",
                       breaks=c("initial_prev_patent", "initial_prev_2_10_patent"),
                       labels=c("Age 0-99", "Age 2-10")) + ggtitle(expression(paste("Patent ", italic("Pf"), "PR", " detected with RDT")))
#theme(axis.text.x = element_text(size=8, angle=45))
ggsave(filename = "~/MMC/TPP/figures/simulation_desc/EIR_prevalence_patent_RDT.pdf",
       width = 8, height = 4.5)
write.table(stats_table, "~/MMC/TPP/figures/simulation_desc/prev_red/prev_distr_patent_RDT.txt", quote = FALSE,
