...
table_file = "~/MMC/TPP/simulations/MAB_twice_sync_3years/param_tab.txt"
ranges_file = "~/MMC/TPP/simulations/MAB_twice_sync_3years/param_ranges.RData"
...
Coverage = c(0, 1)
Access = c(0, 0.5)
Xcand = as.data.frame(lhs(500, rbind(EIR, Halflife, Efficacy, Coverage, Access)))
colnames(Xcand) = c("EIR", "Halflife", "Efficacy", "Coverage", "Access")
SEED = c(1:10)

# Data frame with parameter ranges for the experiment
param_ranges = rbind(c(1, 25), c(0.167, 0.667), c(0.3, 0.95), c(0, 1), c(0, 0.5))
row.names(param_ranges) = c("EIR", "Halflife", "Efficacy", "Coverage", "Access")
